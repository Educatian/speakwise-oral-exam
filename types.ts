...
// BOOTSTRAP fallback admin email — local/demo mode (Supabase NOT configured)
// ONLY. When Supabase is configured, the DB-backed user_profiles.role is the
// single source of truth for admin/instructor authority; never compare
// against this constant directly — go through lib/utils/adminAccess.ts
// (admin UI gating) or lib/supabase/database.ts checkInstructorStatus
// (instructor checks, with its own FALLBACK_INSTRUCTORS bootstrap list).
