...
- **Hash course passwords**: store `courses.password` as `crypt(...)`, add a `verify_course_passcode(course_id, code) returns boolean` RPC, and **remove `password` from the `getAllCourses` SELECT projection** (`database.ts`) so it never ships to the browser. Update `StudentLoginView`'s client-side `selectedCourse.password === passcode` check to call the RPC. *(Apply the migration and ship the client change together, or gate the client on RPC-present to degrade gracefully.)*
